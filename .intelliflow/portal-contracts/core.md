...
- The App runs behind IntelliFlow's **TLS-terminating reverse proxy**, which forwards over plain HTTP internally. The platform code owns this too: `AddPlatformAuthentication`/`UsePlatformAuthentication` honour `X-Forwarded-Proto` (forwarded headers) so `Request.Scheme` is the original **https** — this is what makes the OIDC `redirect_uri` match the https URI the portal registers in Entra. Do not add, duplicate, or remove forwarded-headers handling; do not build redirect/absolute URLs off a hardcoded scheme.
